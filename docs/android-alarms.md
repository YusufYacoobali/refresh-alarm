...
- **Ring again if I drift off** defaults on. Entering a mission starts a 60-second inactivity deadline in the native service. Only answers, valid card flips, and accepted shakes renew it; silence heartbeats do not. The deadline survives service recreation. On expiry the service restores the chosen volume immediately, reposts the full-screen notification, and changes the occurrence ID so the app returns to ringing and restarts the mission sequence. Stop/snooze destroys the timer with the service. A foreground JavaScript equivalent supports browser flow verification.
