...
export function useMissionReminder(alarm: Alarm | undefined, preview: boolean, eventId: string | undefined, missionStep: number | string, completed: boolean,
  previewOptions?: { kind?: Challenge; difficulty?: Alarm["difficulty"]; rounds?: string }) {
  const { kind, difficulty, rounds } = previewOptions ?? {};
    ...
    const check = () => {
      ...
      if (remaining === 0) {
        ...
        router.replace({ pathname: "/ringing", params: {
          ...
          ...(preview ? { kind, difficulty, rounds } : {}),
        } });
      }
    };
  ...
  }, [enabled, alarm?.id, preview, eventId, missionStep, kind, difficulty, rounds]));
